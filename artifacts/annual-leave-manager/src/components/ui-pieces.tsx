...
export function StatusBadge({ status }: { status: LeaveRequestStatus }) {
  const tone = status === 'approved' ? 'bg-[hsl(var(--secondary))] text-[hsl(var(--foreground))]' : status === 'rejected' ? 'bg-[hsl(var(--destructive)/.15)] text-[hsl(var(--destructive))]' : status === 'cancelled' ? 'bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground))]' : 'bg-[hsl(var(--accent)/.2)] text-[hsl(var(--accent))]';
  return <span data-testid={`status-badge-${status}`} className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ${tone}`}><span className={`h-1.5 w-1.5 rounded-full ${status === 'approved' ? 'bg-[hsl(var(--foreground))]' : status === 'rejected' ? 'bg-[hsl(var(--destructive))]' : status === 'cancelled' ? 'bg-[hsl(var(--muted-foreground))]' : 'bg-[hsl(var(--accent))]'}`} />{statusLabels[status]}</span>;
}

export function Avatar({ name, color = 'teal' }: { name: string; color?: 'teal' | 'peach' | 'gold' | 'sage' }) {
  const colors = { teal: 'bg-[hsl(var(--primary)/.1)] text-[hsl(var(--primary))]', peach: 'bg-[hsl(var(--destructive)/.15)] text-[hsl(var(--destructive))]', gold: 'bg-[hsl(var(--accent)/.2)] text-[hsl(var(--accent))]', sage: 'bg-[hsl(var(--secondary)/.5)] text-[hsl(var(--foreground))]' };
...
export function ActionButton({ children, onClick, disabled, kind = 'primary', testId }: { children: React.ReactNode; onClick: () => void; disabled?: boolean; kind?: 'primary' | 'quiet' | 'danger'; testId: string }) {
  const styles = kind === 'primary' ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] hover:brightness-110' : kind === 'danger' ? 'border border-[hsl(var(--destructive-border))] text-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive)/.1)]' : 'border border-[hsl(var(--border))] text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))]';
