...
export default function RequestsPage() {
  ...
  return (
    <div>
      <PageHeader eyebrow="휴가 신청 · 승인 대기" title="승인함" description="구성원의 휴가 신청을 검토하고 처리하세요." action={<div className="hidden items-center gap-2 sm:flex"><span className="h-2 w-2 rounded-full bg-[#d39b2f]" /><span className="text-[12px] font-semibold">{query.data?.filter((request) => request.status === 'pending').length || 0}건 대기 중</span></div>} />
      <div className="mx-auto max-w-[1440px] px-5 py-7 sm:px-8 md:px-10 md:py-9">
        {notice && <div data-testid="status-success-notice" className="mb-5 flex items-center gap-2 rounded-xl border border-[#bddcc4] bg-[#edf7ee] px-4 py-3 text-[12px] font-bold text-[#27643d]"><Check size={16} />{notice}</div>}
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full sm:max-w-[300px]"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[hsl(var(--muted-foreground))]" /><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="이름 또는 부서 검색" data-testid="input-search-requests" className="h-10 w-full rounded-lg border border-[hsl(var(--input))] bg-[hsl(var(--card))] pl-9 pr-3 text-[12px] outline-none transition focus:border-[hsl(var(--primary))] focus:ring-2 focus:ring-[hsl(var(--ring)/.12)]" /></div>
...
function RequestRow({ request, onOpen, onApprove, onReject, busy }: { request: LeaveRequest; onOpen: () => void; onApprove: () => void; onReject: () => void; busy: boolean }) {
  ...
  return <div data-testid={`row-request-${request.id}`} className="group grid cursor-pointer grid-cols-1 gap-3 border-b border-[hsl(var(--border))] px-4 py-4 transition-colors last:border-0 hover:bg-[hsl(var(--muted)/.35)] md:grid-cols-[1.35fr_1fr_.7fr_.7fr_1.1fr] md:items-center md:px-5 md:py-3.5" onClick={onOpen}>
    <div className="flex items-center gap-3"><Avatar name={request.employeeName} color={request.id % 3 === 0 ? 'gold' : request.id % 2 ? 'teal' : 'peach'} /><div><div className="flex items-center gap-2 text-[12px] font-bold">{request.employeeName}<span className="text-[10px] font-normal text-[hsl(var(--muted-foreground))]">{request.department}</span></div><div className="mt-1 text-[10px] text-[hsl(var(--muted-foreground))]">신청일 {formatShortDate(request.createdAt)}</div></div><ChevronRight size={15} className="ml-auto text-[hsl(var(--muted-foreground))] md:hidden" /></div>
